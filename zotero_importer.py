...
class ZoteroImporter:
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.library_id_entry = ttk.Entry(zotero_frame)
            self.library_id_entry.grid(row=0, column=1, padx=5, pady=5)
            
            ttk.Label(zotero_frame, text="API Key:").grid(row=1, column=0, padx=5, pady=5)
            self.api_key_entry = ttk.Entry(zotero_frame, show="*")
            ...
            self.openai_key_entry = ttk.Entry(openai_frame, show="*")
            ...
            self.firecrawl_key_entry = ttk.Entry(firecrawl_frame, show="*")
            self.firecrawl_key_entry.grid(row=0, column=1, padx=5, pady=5)
            ...
            if os.path.exists('zotero_credentials.json'):
                ...
                with open('zotero_credentials.json', 'r', encoding='utf-8') as f:
                    ...
                    if all(hasattr(self, attr) for attr in ['library_id', 'api_key', 'openai_key', 'firecrawl_key']):
                        ...
                        for widget in [self.library_id_entry, self.api_key_entry, self.openai_key_entry, self.firecrawl_key_entry]:
                            if widget and widget.winfo_exists():
                                widget.delete(0, tk.END)
                        
                        # Inserir novos valores
                        self.library_id_entry.insert(0, creds.get('library_id', ''))
                        self.api_key_entry.insert(0, creds.get('api_key', ''))
                        self.openai_key_entry.insert(0, creds.get('openai_key', ''))
                        self.firecrawl_key_entry.insert(0, creds.get('firecrawl_key', ''))
    ...
    def save_credentials(self):
        ...
        try:
            ...
            creds = {
                ...
                'firecrawl_key': self.firecrawl_key_entry.get()
            }
    ...
    def parse_references(self, text):
        ...
        client = OpenAI(api_key=self.openai_key_entry.get())
    ...
    def create_zotero_items(self, items):
        ...
        zot = zotero.Zotero(self.library_id_entry.get(), 'user', self.api_key_entry.get())
    ...
    def fetch_firecrawl_data(self, query):
        ...
        try:
            app = FirecrawlApp(api_key=self.firecrawl_key_entry.get())
    ...
    def import_references(self):
        ...
        if not all([self.library_id_entry.get(), self.api_key_entry.get(), self.openai_key_entry.get(), self.firecrawl_key_entry.get()]):
